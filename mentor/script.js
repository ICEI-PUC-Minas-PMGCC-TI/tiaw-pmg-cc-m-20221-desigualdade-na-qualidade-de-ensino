...
if (localStorage.getItem('token') == null) { // verificação de login
  ...
  afazeres.href = 'mentor/login-registro/login.html';
  perfil.href = 'mentor/login-registro/login.html';
  log.addEventListener('click', () => {
    window.location.href = 'mentor/login-registro/login.html';
  })
} else { // logout
  log.innerHTML = `
  <ul>
  ...
  afazeres.href = 'mentor/planner/todolist.html';
  perfil.href = 'mentor/perfil/perfil.html';
  log.addEventListener('click', () => {
...
function addDicas() { // adicionando dicas
  ...
  for (let i = 0; i < arrayTitulo.length; i++) {
    ...
    document.getElementById(i).addEventListener('click', () => { // abrir dicas
      ...
      window.location.href = 'mentor/Dicas/dica.html'
    });
  ...
  if (todo == null) {
    if(localStorage.getItem('token') == null){
      li = document.createElement('li');
        li.innerHTML = `
        <div class="divLi">
          <div ><a href="mentor/login-registro/login.html">Logue</a> para ver a suas tarefas</div>
        </div>
      `
        ul2.appendChild(li);
    }else{
